...
async def process_successful_payment(message: Message):
    try:
        with connection.cursor() as cursor:
            ...
            edit_team = f'UPDATE users SET team = {game_users[str(message.from_user.id)]["pre_team"]} WHERE id = {message.from_user.id};'
            cursor.execute(edit_payed)
            connection.commit()
            cursor.execute(edit_team)
            connection.commit()

        game_users[str(message.from_user.id)]['payed'] = True
        game_users[str(message.from_user.id)]['team'] = game_users[str(message.from_user.id)]['pre_team']
